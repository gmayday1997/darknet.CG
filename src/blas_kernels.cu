...
__global__ void channel_slice_kernel(int count, float *output,float *input, int batch_size,int spatial_size,int input_slice_axis,int output_slice_axis,int begin_slice_axis,int bForward)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(index > count) return;
    int feature_map_size = spatial_size * output_slice_axis;
...
//added by cheerguo 2019/08/02
extern "C" void channel_slice_ongpu(int count,float *output,float *input, int batch_size,int spatial_size,int input_slice_axis,int output_slice_axis,int begin_slice_axis, int bForward)
{

   channel_slice_kernel<<<cuda_gridsize(count), BLOCK, 0, get_cuda_stream()>>>(count,output,input,batch_size,spatial_size,input_slice_axis,\
   output_slice_axis, begin_slice_axis,bForward);
   CHECK_CUDA(cudaPeekAtLastError()); 
}
///added by cheerguo 2019/07/31
__global__ void channel_shuffle_kernel(int count, float *output,float *input, int group_row,int group_column, int feature_map_size,int spatial_size)
{
    int index = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(index > count) return;
    const int n = index / group_row / group_column / feature_map_size;
    const int i = (index / group_column / spatial_size) % group_row;
    const int j = index / spatial_size % group_column;
    const int k = index - (n * feature_map_size + (i * group_column + j) * spatial_size);
    float *p_o = output + n * feature_map_size + (j * group_row + i) * spatial_size;
    p_o[k] = input[index];
}

extern "C" void channel_shuffle_ongpu(int N,float *output,float *input, int group_row,int group_column,int feature_map_size, int len)
{
    channel_shuffle_kernel<<<cuda_gridsize(N), BLOCK, 0, get_cuda_stream()>>>(N, output, input, group_row, group_column, feature_map_size,len);
    CHECK_CUDA(cudaPeekAtLastError());
}
